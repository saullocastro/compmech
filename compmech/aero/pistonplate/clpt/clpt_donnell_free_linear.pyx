            ...
            for k1 in range(1, m1+1):
                for l1 in range(1, n1+1):
                    ...
                    if k1 != i1 and l1 != j1:
                        ...
                        k0r[c] = row+1
                        k0c[c] = col+3
                        k0v[c] += 0.5*(pi*pi)*((-1)**(i1 + k1) - 1)*(B16*(b*b)*(i1*i1)*(k1*k1) + B26*(a*a)*(j1*j1)*((i1*i1) + 2*(k1*k1)))/((a*a)*b*(i1 - k1)*(i1 + k1))
                        c += 1
                        k0r[c] = row+2
                        k0c[c] = col+0
                        ...
                        k0r[c] = row+3
                        k0c[c] = col+0
                        k0v[c] += -0.5*(pi*pi)*((-1)**(i1 + k1) - 1)*(B11*(b*b)*(i1*i1)*(k1*k1) + (a*a)*(j1*j1)*(B12*(k1*k1) + 2*B66*(i1*i1)))/((a*a)*b*(i1 - k1)*(i1 + k1))
                        c += 1
                        k0r[c] = row+3
                        k0c[c] = col+1
                        ...
                        k0r[c] = row+2
                        k0c[c] = col+3
                        k0v[c] += (pi*pi*pi*pi)*i1*j1*(-D16*(i1*i1)/(a*a) - D26*(j1*j1)/(b*b))
                        c += 1
                        k0r[c] = row+3
                        k0c[c] = col+2
                        k0v[c] += (pi*pi*pi*pi)*i1*j1*(-D16*(i1*i1)/(a*a) - D26*(j1*j1)/(b*b))
...
def fk0edges(int m1, int n1, double a, double b,
    ...
    fdim = 4*m1*n1*m1*n1 + 4*m1*n1*m1*n1
            ...
            for k1 in range(1, m1+1):
                for l1 in range(1, n1+1):
                    ...
                    if k1 != i1 and l1 != j1:
                        ...
                        kG0r[c] = row+2
                        kG0c[c] = col+3
                        kG0v[c] += -0.5*(pi*pi)*i1*j1*(Fxy*a + Fyx*b)/(a*b)
                        c += 1
                        kG0r[c] = row+3
                        kG0c[c] = col+2
                        kG0v[c] += -0.5*(pi*pi)*i1*j1*(Fxy*a + Fyx*b)/(a*b)
            ...
            for k1 in range(1, m1+1):
                for l1 in range(1, n1+1):
                    ...
                    if k1 != i1 and l1 != j1:
                        ...
                        kAr[c] = row+2
                        kAc[c] = col+3
                        kAv[c] += -beta*b*i1*j1*((-1)**(j1 + l1) - 1)/(2.0*(j1*j1) - 2.0*(l1*l1))
                        c += 1
                        kAr[c] = row+3
                        kAc[c] = col+2
                        kAv[c] += beta*b*i1*l1*((-1)**(j1 + l1) - 1)/(-2.0*(j1*j1) + 2.0*(l1*l1))
                        ...
                        kAr[c] = row+3
                        kAc[c] = col+3
                        kAv[c] += beta*b*(i1*i1)*((-1)**(i1 + k1) - 1)/(2.0*(i1*i1) - 2.0*(k1*k1))
            ...
            for k1 in range(1, m1+1):
                for l1 in range(1, n1+1):
                    ...
                    if k1 != i1 and l1 != j1:
                        ...
                        kMr[c] = row+2
                        kMc[c] = col+3
                        kMv[c] += a*b*h*i1*j1*mu*((-1)**(i1 + k1) - 1)*((-1)**(j1 + l1) - 1)/((pi*pi)*((i1*i1) - (k1*k1))*((j1*j1) - (l1*l1)))
                        c += 1
                        kMr[c] = row+3
                        kMc[c] = col+2
                        kMv[c] += a*b*h*k1*l1*mu*((-1)**(i1 + k1) - 1)*((-1)**(j1 + l1) - 1)/((pi*pi)*((i1*i1) - (k1*k1))*((j1*j1) - (l1*l1)))
