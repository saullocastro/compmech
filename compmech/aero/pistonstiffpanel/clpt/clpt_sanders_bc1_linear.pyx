            ...
            for k1 in range(1, m1+1):
                for l1 in range(1, n1+1):
                    ...
                    if k1 != i1 and l1 != j1:
                        ...
                        k0r[c] = row+0
                        k0c[c] = col+1
                        k0v[c] += -i1*j1*k1*l1*((-1)**(i1 + k1) - 1)*((-1)**(j1 + l1) - 1)*(B12 + B66 + r*(A12 + A66))/(r*((i1*i1) - (k1*k1))*((j1*j1) - (l1*l1)))
                        c += 1
                        k0r[c] = row+1
                        k0c[c] = col+0
                        k0v[c] += -i1*j1*k1*l1*((-1)**(i1 + k1) - 1)*((-1)**(j1 + l1) - 1)*(B12 + B66 + r*(A12 + A66))/(r*((i1*i1) - (k1*k1))*((j1*j1) - (l1*l1)))
                        ...
                        k0r[c] = row+1
                        k0c[c] = col+1
                        k0v[c] += i1*j1*k1*l1*(-2*(-1)**(i1 + k1) + 2)*((-1)**(j1 + l1) - 1)*(D26 + r*(A26*r + 2*B26))/((r*r)*((i1*i1) - (k1*k1))*((j1*j1) - (l1*l1)))
                        c += 1
                        k0r[c] = row+2
                        k0c[c] = col+2
                        ...
                        k0r[c] = row+0
                        k0c[c] = col+2
                        k0v[c] += 0.5*j1*l1*((-1)**(j1 + l1) - 1)*(3*(pi*pi)*B16*(b*b)*(i1*i1)*r + (a*a)*(A26*(b*b) + (pi*pi)*B26*(l1*l1)*r))/(a*(b*b)*r*((j1*j1) - (l1*l1)))
                        c += 1
                        k0r[c] = row+1
                        k0c[c] = col+2
                        k0v[c] += 0.5*j1*l1*((-1)**(j1 + l1) - 1)*((a*a)*((b*b)*(A22*r + B22) + (pi*pi)*(l1*l1)*r*(B22*r + D22)) + (pi*pi)*(b*b)*(i1*i1)*r*(B12*r + 2*B66*r + D12 + 2*D66))/(a*(b*b)*(r*r)*((j1*j1) - (l1*l1)))
                        c += 1
                        k0r[c] = row+2
                        k0c[c] = col+0
                        ...
                        k0r[c] = row+2
                        k0c[c] = col+1
                        k0v[c] += -0.5*j1*l1*((-1)**(j1 + l1) - 1)*((a*a)*((b*b)*(A22*r + B22) + (pi*pi)*(j1*j1)*r*(B22*r + D22)) + (pi*pi)*(b*b)*(i1*i1)*r*(B12*r + 2*B66*r + D12 + 2*D66))/(a*(b*b)*(r*r)*((j1*j1) - (l1*l1)))
                        ...
                        k0r[c] = row+1
                        k0c[c] = col+2
                        k0v[c] += 0.5*i1*k1*((-1)**(i1 + k1) - 1)*((a*a)*((b*b)*(A26*r + B26) + 3*(pi*pi)*(j1*j1)*r*(B26*r + D26)) + (pi*pi)*(b*b)*(k1*k1)*r*(B16*r + D16))/((a*a)*b*(r*r)*((i1*i1) - (k1*k1)))
                        c += 1
                        k0r[c] = row+2
                        k0c[c] = col+0
                        ...
                        k0r[c] = row+2
                        k0c[c] = col+1
                        k0v[c] += -0.5*i1*k1*((-1)**(i1 + k1) - 1)*((a*a)*((b*b)*(A26*r + B26) + 3*(pi*pi)*(j1*j1)*r*(B26*r + D26)) + (pi*pi)*(b*b)*(i1*i1)*r*(B16*r + D16))/((a*a)*b*(r*r)*((i1*i1) - (k1*k1)))
                        ...
                        k0r[c] = row+0
                        k0c[c] = col+1
                        k0v[c] += 0.25*(pi*pi)*((a*a)*(j1*j1)*(A26*r + B26) + (b*b)*(i1*i1)*(A16*r + B16))/(a*b*r)
                        c += 1
                        k0r[c] = row+1
                        k0c[c] = col+0
                        k0v[c] += 0.25*(pi*pi)*((a*a)*(j1*j1)*(A26*r + B26) + (b*b)*(i1*i1)*(A16*r + B16))/(a*b*r)
                        ...
                        k0r[c] = row+1
                        k0c[c] = col+1
                        k0v[c] += 0.25*(pi*pi)*((a*a)*(j1*j1)*(D22 + r*(A22*r + 2*B22)) + (b*b)*(i1*i1)*(D66 + r*(A66*r + 2*B66)))/(a*b*(r*r))
                        c += 1
                        k0r[c] = row+2
                        k0c[c] = col+2
