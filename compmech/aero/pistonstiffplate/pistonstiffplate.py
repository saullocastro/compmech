...
def load(name):
    if '.AeroPistonStiffPlate' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.AeroPistonStiffPlate', 'rb'))
...
class Stiffener(object):
    ...
    :class:`.AeroPistonStiffPlate` object may have any number of stiffeners.

    Each stiffener has a constant `y` coordinate.

    .. note:: This class essentially works as the
              :class:`.AeroPistonStiffPanel` with `r \rightarrow \infty`.
...
class AeroPistonStiffPlate(object):
    r"""Stiffened Plate for Aeroelastic Studies using the Piston Theory
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('AeroPistonStiffPlate name unchanged')
    ...
    def calc_linear_matrices(self, combined_load_case=None, silent=False,
        ...
        if calc_kG0:
            ...
            if not combined_load_case:
                ...
                kG0_Nxx = fkG0(Nxx, 0, 0, a, b, m1, n1)
                kG0_Nyy = fkG0(0, Nyy, 0, a, b, m1, n1)
                kG0_Nxy = fkG0(0, 0, Nxy, a, b, m1, n1)
    ...
    def lb(self, tol=0, combined_load_case=None, sparse_solver=True,
        ...
        The following parameters of the ``AeroPistonStiffPlate`` object will
        ...
        of the ``AeroPistonStiffPlate`` object and the `i^{th}` eigenvector in
    ...
    def plot(self, c, invert_y=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``AeroPistonStiffPlate`` object will
            be used.
    ...
    def save(self):
        """Save the ``AeroPistonStiffPlate`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in
        ``AeroPistonStiffPlate.name`` followed by a
        ``'.AeroPistonStiffPlate'`` extension.

        """
        name = self.name + '.AeroPistonStiffPlate'
        msg('Saving AeroPistonStiffPlate to {0}'.format(name))
