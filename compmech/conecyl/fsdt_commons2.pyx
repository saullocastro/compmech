...
cdef cfg(double[:, ::1] g, int m1, int m2, int n2,
    ...
    for i2 in range(1, m2+1):
        ...
        for j2 in range(1, n2+1):
            ...
            g[2, col+6] = sinbi*sinbj
            g[2, col+7] = sinbi*cosbj

            g[3, col+8] = sinbi*sinbj
            g[3, col+9] = sinbi*cosbj
            g[3, col+10] = cosbi*sinbj
            g[3, col+11] = cosbi*cosbj

            g[4, col+12] = sinbi*sinbj
            g[4, col+13] = sinbi*cosbj
...
cdef void *cfstrain_donnell(double *c, double sina, double cosa, double tLA,
    ...
    for i1 in range(1, m1+1):
        ...
        etz += (pi*c[col+2]*cosi1x*i1/L
                + c[col+3]*sini1x
                + c[col+4]*cosi1x)

        exz += (-c[col+1]*cosa*sini1x/r
                + c[col+5]*sini1x)
    ...
    for j2 in range(1, n2+1):
        ...
        for i2 in range(1, m2+1):
            ...
            kxx += (pi*c[col+9]*cosi2x*cosj2t*i2/L
                    + pi*c[col+8]*cosi2x*i2*sinj2t/L
