...
cdef void cfwx(double *c, int m1, int m2, int n2, double L,
    ...
    cdef int i1, i2, j2, col
    cdef double sinbj, cosbj, dsinbi, dcosbi, wx
    ...
    for j2 in range(1, n2+1):
        ...
        for i2 in range(1, m2+1):
            ...
            wx += c[col+8]*dsinbi*sinbj
            wx += c[col+9]*dsinbi*cosbj
            wx += c[col+10]*dcosbi*sinbj
            wx += c[col+11]*dcosbi*cosbj
...
cdef void cfwt(double *c, int m1, int m2, int n2, double L,
    ...
    for j2 in range(1, n2+1):
        ...
        for i2 in range(1, m2+1):
            ...
            sinbi = sin(i2*pi*x/L)
            cosbi = cos(i2*pi*x/L)
            wt += c[col+8]*sinbi*dsinbj
            wt += c[col+9]*sinbi*dcosbj
            wt += c[col+10]*cosbi*dsinbj
            wt += c[col+11]*cosbi*dcosbj
...
cdef cfg(double[:, ::1] gss, int m1, int m2, int n2,
    ...
    for i2 in range(1, m2+1):
        ...
        for j2 in range(1, n2+1):
            ...
            gss[1, col+6] = cosbi*sinbj
            gss[1, col+7] = cosbi*cosbj

            gss[2, col+8] = sinbi*sinbj
            gss[2, col+9] = sinbi*cosbj
            gss[2, col+10] = cosbi*sinbj
            gss[2, col+11] = cosbi*cosbj

            gss[3, col+12] = sinbi*sinbj
            gss[3, col+13] = sinbi*cosbj
            gss[3, col+14] = cosbi*sinbj
            gss[3, col+15] = cosbi*cosbj

            gss[4, col+16] = sinbi*sinbj
            gss[4, col+17] = sinbi*cosbj
            gss[4, col+18] = cosbi*sinbj
            gss[4, col+19] = cosbi*cosbj
...
cdef void *cfstrain_donnell(double *c, double sina, double cosa, double tLA,
    ...
    for i1 in range(1, m1+1):
        ...
        kxx += (pi*c[col+3]*cosi1x*i1/L
                -pi*c[col+4]*i1*sini1x/L)

        ktt += (c[col+3]*sina*sini1x/r
                + c[col+4]*cosi1x*sina/r)

        kxt += c[col+5]*(-sina*sini1x/r + pi*cosi1x*i1/L)

        etz += (-c[col+1]*cosa*sini1x/r
                + c[col+5]*sini1x)

        exz += (pi*c[col+2]*cosi1x*i1/L
                + c[col+3]*sini1x
                + c[col+4]*cosi1x)
    ...
    for j2 in range(1, n2+1):
        ...
        for i2 in range(1, m2+1):
            ...
            ett += (c[col+0]*sina*sini2x*sinj2t/r
                    ...
                    + 0.5*c[col+4]*sini2x*(2*cosa*r*sinj2t + cosj2t*j2*wt)/r**2
                    + 0.5*c[col+5]*sini2x*(2*cosa*cosj2t*r - j2*sinj2t*wt)/r**2)
            ...
            kxx += (pi*c[col+6]*cosi2x*i2*sinj2t/L
                    + pi*c[col+7]*cosi2x*cosj2t*i2/L
                    -pi*c[col+8]*i2*sini2x*sinj2t/L
                    -pi*c[col+9]*cosj2t*i2*sini2x/L)

            ktt += (c[col+6]*sina*sini2x*sinj2t/r
                    + c[col+7]*cosj2t*sina*sini2x/r
                    + c[col+8]*cosi2x*sina*sinj2t/r
                    + c[col+9]*cosi2x*cosj2t*sina/r
                    + c[col+10]*cosj2t*j2*sini2x/r
                    -c[col+11]*j2*sini2x*sinj2t/r)

            kxt += (c[col+6]*cosj2t*j2*sini2x/r
                    -c[col+7]*j2*sini2x*sinj2t/r
                    + c[col+8]*cosi2x*cosj2t*j2/r
                    -c[col+9]*cosi2x*j2*sinj2t/r
                    + c[col+10]*sinj2t*(-L*sina*sini2x + pi*cosi2x*i2*r)/(L*r)
                    + c[col+11]*cosj2t*(-L*sina*sini2x + pi*cosi2x*i2*r)/(L*r))

            etz += (-c[col+2]*cosa*sini2x*sinj2t/r
                    -c[col+3]*cosa*cosj2t*sini2x/r
                    + c[col+4]*cosj2t*j2*sini2x/r
                    -c[col+5]*j2*sini2x*sinj2t/r
                    + c[col+10]*sini2x*sinj2t
                    + c[col+11]*cosj2t*sini2x)

            exz += (pi*c[col+4]*cosi2x*i2*sinj2t/L
                    + pi*c[col+5]*cosi2x*cosj2t*i2/L
                    + c[col+6]*sini2x*sinj2t
                    + c[col+7]*cosj2t*sini2x
