...
def load(name):
    if '.KPanelCheb' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.KPanelCheb', 'rb'))


class KPanelCheb(object):
    r"""Conical (Konus) panel using Chebyshev polynomials

    The approximation functions for the displacement field are Chebyshev
    polynomials of the second kind:
        ...
        .. math::
            ...
            ...
            \\
            u_i(x) = 2 x u_{i-1} - u_{i-2}
            \\
            u_j(\theta) = 2 \theta u_{j-1} - u_{j-2}

    such that the 2-dimensional approximation functions are built doing:

        .. math::
            u_{ij}(x, \theta) = u_i(x) u_j(\theta)
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('KPanelCheb name unchanged')
    ...
    def lb(self, tol=0, combined_load_case=None, remove_null_i1_j1=False,
        ...
        The following parameters of the ``KPanelCheb`` object will affect the
        ...
        of the ``KPanelCheb`` object and the `i^{th}` eigenvector in the
    ...
    def uvw(self, c, xs=None, ts=None, gridx=300, gridt=300):
        ...
        ``u``, ``v``, ``w``, ``phix``, ``phit`` of the ``KPanelCheb`` object.
        ...
        stored as parameters with the same name in the ``KPanelCheb`` object.
    ...
    def add_SPL(self, PL, pt=0.5, theta=0., cte=True):
        ...
        the ``KPanelCheb`` object if ``cte=True``, or to the ``forces_inc``
    ...
    def plot(self, c, invert_theta=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``KPanelCheb`` object will be used.
    ...
    def save(self):
        """Save the ``KPanelCheb`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in ``KPanelCheb.name``
        followed by a ``'.KPanelCheb'`` extension.

        """
        name = self.name + '.KPanelCheb'
        msg('Saving KPanelCheb to {}'.format(name))
