...
cdef void cfuvw(double *c, int m2, int n3, int m4, int n4,
    ...
    cdef double cosi2bx, cosj3bt, cosi4bx, cosj4bt
    ...
    for i in range(size):
        ...
        for i2 in range(1, m2+1):
            ...
            v += c[col+1]*cosi2bx
            w += c[col+2]*cosi2bx
            phix += c[col+3]*cosi2bx
            phit += c[col+4]*cosi2bx
        ...
        for j3 in range(1, n3+1):
            ...
            v += c[col+1]*cosj3bt
            w += c[col+2]*cosj3bt
            phix += c[col+3]*cosj3bt
            phit += c[col+4]*cosj3bt
        ...
        for j4 in range(1, n4+1):
            ...
            for i4 in range(1, m4+1):
                ...
                u += c[col+0]*cosi4bx*cosj4bt
                v += c[col+1]*cosi4bx*cosj4bt
                w += c[col+2]*cosi4bx*cosj4bt
...
cdef void cfwx(double *c, int m2, int n3, int m4, int n4, double *xs, double
    *ts, int size, double L, double tmin, double tmax, double *outwx) nogil:
    cdef double dcosi2bx, dcosi4bx, cosj4bt, wx, x, t, bx, bt
    cdef int i2, j3, i4, j4, col, i
    ...
    for i in range(size):
        ...
        for j4 in range(1, n4+1):
            ...
            for i4 in range(1, m4+1):
                col = (num0 + num1 + num2*m2 + num3*n3 + (j4-1)*num4*m4 +
                        (i4-1)*num4)
                dcosi4bx = -i4*pi/L*sin(i4*pi*bx)
                wx += dcosi4bx*cosj4bt*c[col+2]
...
cdef void cfwt(double *c, int m2, int n3, int m4, int n4, double *xs, double
    *ts, int size, double L, double tmin, double tmax, double *outwt) nogil:
    cdef double dcosj3bt, cosi4bx, dcosj4bt, wt, x, t, bx, bt
    cdef int i2, j3, i4, j4, col, i
    ...
    for i in range(size):
        ...
        for j3 in range(1, n3+1):
            dcosj3bt = -j3*pi/(tmax-tmin)*sin(j3*pi*bt)
            col = num0 + num1 + num2*m2 + num3*(j3-1)
            wt += c[col+2]*dcosj3bt

        for j4 in range(1, n4+1):
            dcosj4bt = -j4*pi/(tmax-tmin)*sin(j4*pi*bt)
            for i4 in range(1, m4+1):
                col = (num0 + num1 + num2*m2 + num3*n3 + (j4-1)*num4*m4 +
                        (i4-1)*num4)
                cosi4bx = cos(i4*pi*bx)
                wt += cosi4bx*dcosj4bt*c[col+2]
    ...
    for j4 in range(1, n4+1):
        ...
        for i4 in range(1, m4+1):
            col = (num0 + num1 + num2*m2 + num3*n3 + (j4-1)*num4*m4 +
                    (i4-1)*num4)
            cosi4bx = cos(i4*pi*bx)
            g[0, col+0] = cosi4bx*cosj4bt
            g[1, col+1] = cosi4bx*cosj4bt
            g[2, col+2] = cosi4bx*cosj4bt
