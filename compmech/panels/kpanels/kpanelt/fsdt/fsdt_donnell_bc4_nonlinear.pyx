...
from compmech.panels.kpanels.kpanelt.fsdt.fsdt_commons_bc4 cimport (cfwx,
        cfwt, cfN)
...
cdef void cfk0L(int npts, double *xs, double *ts, double *out,
    ...
    cdef double p30, p31, p32, p40, p41, p42
    ...
    for i in range(npts):
        ...
        for i1 in range(m1):
            ...
            for j1 in range(n1):
                ...
                p20 = A12*cosa*sini1bx*sinj1bt
                p21 = A22*cosa*sini1bx*sinj1bt
                p22 = A26*cosa*sini1bx*sinj1bt
                p30 = pi*B16*cosi1bx*j1*sinj1_bt/(tmax - tmin) + cosj1_bt*(-pi*B11*i1*r*sini1bx/L + B12*cosi1bx*sina)
                p31 = pi*B26*cosi1bx*j1*sinj1_bt/(tmax - tmin) + cosj1_bt*(-pi*B12*i1*r*sini1bx/L + B22*cosi1bx*sina)
                p32 = pi*B66*cosi1bx*j1*sinj1_bt/(tmax - tmin) + cosj1_bt*(-pi*B16*i1*r*sini1bx/L + B26*cosi1bx*sina)
                p40 = pi*B12*cosi1bx*j1*sinj1_bt/(tmax - tmin) - B16*cosj1_bt*(L*cosi1bx*sina + pi*i1*r*sini1bx)/L
                p41 = pi*B22*cosi1bx*j1*sinj1_bt/(tmax - tmin) - B26*cosj1_bt*(L*cosi1bx*sina + pi*i1*r*sini1bx)/L
                p42 = pi*B26*cosi1bx*j1*sinj1_bt/(tmax - tmin) - B66*cosj1_bt*(L*cosi1bx*sina + pi*i1*r*sini1bx)/L
    ...
    for i in range(npts):
        ...
        for j1 in range(n1):
            ...
            for i1 in range(m1):
                ...
                kxx0 += -pi*c[col+3]*cosj1_bt*i1*sini1bx/L

                ktt0 += (c[col+3]*cosi1bx*cosj1_bt*sina/r
                         -pi*c[col+4]*cosi1bx*j1*sinj1bt/(r*tmax - r*tmin))

                kxt0 += (-pi*c[col+3]*cosi1bx*j1*sinj1bt/(r*tmax - r*tmin)
                         -c[col+4]*cosj1_bt*(L*cosi1bx*sina + pi*i1*r*sini1bx)/(L*r))

                gtz0 += (-c[col+1]*cosa*cosi1bx*cosj1_bt/r
                         + pi*c[col+2]*cosj1bt*j1*sini1bx/(r*tmax - r*tmin)
                         + c[col+4]*cosi1bx*cosj1_bt)

                gxz0 += (pi*c[col+2]*cosi1bx*i1*sinj1bt/L
                         + c[col+3]*cosi1bx*cosj1_bt)
        ...
        for j1 in range(n1):
            ...
            for i1 in range(m1):
                ...
                fint[col+1] = beta*(fint[col+1]) + alpha*((-pi*L*NttL*cosi1bx*j1*sinj1bt + cosj1_bt*(-tmax + tmin)*(L*cosi1bx*(NxtL*sina + QtL*cosa) + pi*NxtL*i1*r*sini1bx))/(L*(tmax - tmin)))
                fint[col+2] = beta*(fint[col+2]) + alpha*((L*sini1bx*(NttL*cosa*r*sinj1bt*(tmax - tmin) + pi*cosj1_bt*j1*(Ntt0*(w0t + wt) + NttL*w0t + NttL*wt + Nxt0*r*w0x + Nxt0*r*wx + NxtL*r*w0x + NxtL*r*wx + QtL*r)) + pi*cosi1bx*i1*r*sinj1bt*(tmax - tmin)*(Nxt0*(w0t + wt) + NxtL*w0t + NxtL*wt + Nxx0*r*w0x + Nxx0*r*wx + NxxL*r*w0x + NxxL*r*wx + QxL*r))/(L*r*(tmax - tmin)))
                fint[col+3] = beta*(fint[col+3]) + alpha*((-pi*L*MxtL*cosi1bx*j1*sinj1bt + cosj1_bt*(tmax - tmin)*(L*cosi1bx*(MttL*sina + QxL*r) - pi*MxxL*i1*r*sini1bx))/(L*(tmax - tmin)))
                fint[col+4] = beta*(fint[col+4]) + alpha*((-pi*L*MttL*cosi1bx*j1*sinj1bt + cosj1_bt*(tmax - tmin)*(L*cosi1bx*(-MxtL*sina + QtL*r) - pi*MxtL*i1*r*sini1bx))/(L*(tmax - tmin)))
