            ...
            for k1 in range(m1):
                ...
                for j in range(n):
                    for l1 in range(n1):
                        ...
                        kCpbr[c] = row+0
                        kCpbc[c] = col+0
                        kCpbv[c] += -0.25*c1*a*b*fAupBu*gAuqBu*kt
                        c += 1
                        kCpbr[c] = row+1
                        kCpbc[c] = col+1
                        kCpbv[c] += -0.25*c1*a*b*fAvpBv*gAvqBv*kt
                        c += 1
                        kCpbr[c] = row+2
                        kCpbc[c] = col+0
                        kCpbv[c] += -0.5*c1*b*dpb*fAwxipBu*gAwqBu*kt
                        c += 1
                        kCpbr[c] = row+2
                        kCpbc[c] = col+1
                        ...
                        kCpbr[c] = row+2
                        kCpbc[c] = col+2
                        kCpbv[c] += -0.25*c1*a*b*fAwpBw*gAwqBw*kt
...
def fkCbbpbx1x2(double x1, double x2, double y1, double y2,
        double kt, double kr, double a, double b,
        int m1, int n1,
        double u1txb, double u1rxb, double u2txb, double u2rxb,
        double v1txb, double v1rxb, double v2txb, double v2rxb,
        double w1txb, double w1rxb, double w2txb, double w2rxb,
            ...
            for k1 in range(m1):
                ...
                for j1 in range(n1):
                    for l1 in range(n1):
                        ...
                        kCbbpbr[c] = row+0
                        kCbbpbc[c] = col+0
                        kCbbpbv[c] += 0.25*c1*a*b*kt*pAupBu*qAuqBu
                        c += 1
                        kCbbpbr[c] = row+1
                        kCbbpbc[c] = col+1
                        kCbbpbv[c] += 0.25*c1*a*b*kt*pAvpBv*qAvqBv
                        c += 1
                        kCbbpbr[c] = row+2
                        kCbbpbc[c] = col+2
                        kCbbpbv[c] += 0.25*c1*a*b*kt*pAwpBw*qAwqBw
...
def fkCppx1x2(double x1, double x2,
          double kt, double a, double b, double dpb, int m, int n,
          double u1tx, double u1rx, double u2tx, double u2rx,
          double v1tx, double v1rx, double v2tx, double v2rx,
          double w1tx, double w1rx, double w2tx, double w2rx,
...
def fkCpbx1x2(double x1, double x2,
          double kt, double a, double b, double bb, double dpb,
          int m, int n, int m1, int n1,
          double u1tx, double u1rx, double u2tx, double u2rx,
          double v1tx, double v1rx, double v2tx, double v2rx,
          double w1tx, double w1rx, double w2tx, double w2rx,
            ...
            for k1 in range(m1):
                ...
                for j in range(n):
                    for l1 in range(n1):
                        ...
                        gAuqBu = integral_ff(l1, j, u1tyb, u1ryb, u2tyb, u2ryb, u1ty, u1ry, u2ty, u2ry)
                        gAvqBv = integral_ff(l1, j, v1tyb, v1ryb, v2tyb, v2ryb, v1ty, v1ry, v2ty, v2ry)
                        gAwqBw = integral_ff(l1, j, w1tyb, w1ryb, w2tyb, w2ryb, w1ty, w1ry, w2ty, w2ry)
                        gAwetaqBv = integral_ffxi(l1, j, v1tyb, v1ryb, v2tyb, v2ryb, w1ty, w1ry, w2ty, w2ry)
                        gAwqBu = integral_ff(l1, j, u1tyb, u1ryb, u2tyb, u2ryb, w1ty, w1ry, w2ty, w2ry)
                        ...
                        kCpbr[c] = row+2
                        kCpbc[c] = col+0
                        kCpbv[c] += -0.5*bb*dpb*fAwxipBu*gAwqBu*kt
                        c += 1
                        kCpbr[c] = row+2
                        kCpbc[c] = col+1
                        kCpbv[c] += -0.5*a*bb*dpb*fAwpBv*gAwetaqBv*kt/b
                        c += 1
                        kCpbr[c] = row+2
                        kCpbc[c] = col+2
