...
def load(name):
    if '.StiffPanel' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.StiffPanel', 'rb'))
...
class Stiffener(object):
    ...
    :class:`.StiffPanel` object may have any number of stiffeners.
...
class StiffPanel(object):
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('StiffPanel name unchanged')
    ...
    def lb(self, tol=0, combined_load_case=None, sparse_solver=True,
        ...
        The following parameters of the ``StiffPanel`` object will
        ...
        of the ``StiffPanel`` object and the `i^{th}` eigenvector in
    ...
    def plot(self, c, invert_y=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``StiffPanel`` object will
            be used.
    ...
    def save(self):
        """Save the ``StiffPanel`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in
        ``StiffPanel.name`` followed by a
        ``'.StiffPanel'`` extension.

        """
        name = self.name + '.StiffPanel'
        msg('Saving StiffPanel to {0}'.format(name))
