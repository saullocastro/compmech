...
def load(name):
    if '.StiffPanelBay1D' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.StiffPanelBay1D', 'rb'))
...
class Stiffener(object):
    ...
    :class:`.StiffPanelBay1D` object may have any number of
    stiffeners.
...
class StiffPanelBay1D(object):
    r"""Stiffened Panel

    Stiffeners modeled using 1D formulation.
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('StiffPanelBay1D name unchanged')
    ...
    def lb(self, tol=0, combined_load_case=None, sparse_solver=True,
        ...
        The following parameters of the ``StiffPanelBay1D`` object
        ...
        of the ``StiffPanelBay1D`` object and the `i^{th}` eigenvector
    ...
    def plot(self, c, invert_y=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``StiffPanelBay1D`` object
            will be used.
    ...
    def save(self):
        """Save the ``StiffPanelBay1D`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in
        ``StiffPanelBay1D.name`` followed by a
        ``'.StiffPanelBay1D'`` extension.

        """
        name = self.name + '.StiffPanelBay1D'
        msg('Saving StiffPanelBay1D to {0}'.format(name))
