...
def load(name):
    if '.StiffPanelBay2D' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.StiffPanelBay2D', 'rb'))
...
class Stiffener2D(object):
    ...
    :class:`.StiffPanelBay2D` object may have any number of
    stiffeners.
...
class StiffPanelBay2D(object):
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('StiffPanelBay2D name unchanged')
    ...
    def lb(self, tol=0, combined_load_case=None, sparse_solver=True,
        ...
        The following parameters of the ``StiffPanelBay2D`` object
        ...
        of the ``StiffPanelBay2D`` object and the `i^{th}`
    ...
    def plot_skin(self, c, invert_y=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``StiffPanelBay2D`` object
            will be used.
    ...
    def save(self):
        """Save the ``StiffPanelBay2D`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in
        ``StiffPanelBay2D.name`` followed by a
        ``'.StiffPanelBay2D'`` extension.

        """
        name = self.name + '.StiffPanelBay2D'
        msg('Saving StiffPanelBay2D to {0}'.format(name))
