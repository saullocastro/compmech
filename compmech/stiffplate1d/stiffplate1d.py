...
def load(name):
    if '.StiffPlate1D' in name:
        return cPickle.load(open(name, 'rb'))
    else:
        return cPickle.load(open(name + '.StiffPlate1D', 'rb'))
...
class Stiffener(object):
    ...
    :class:`.StiffPlate1D` object may have any number of stiffeners.

    Each stiffener has a constant `y` coordinate.

    .. note:: This class essentially works as the
              :class:`.StiffPanel` with `r \rightarrow \infty`.
...
class StiffPlate1D(object):
    ...
    def _rebuild(self):
        if not self.name:
            try:
                ...
                warn('StiffPlate1D name unchanged')
    ...
    def lb(self, tol=0, combined_load_case=None, sparse_solver=True,
        ...
        The following parameters of the ``StiffPlate1D`` object will
        ...
        of the ``StiffPlate1D`` object and the `i^{th}` eigenvector in
    ...
    def plot(self, c, invert_y=False, plot_type=1, vec='w',
        ...
        filename : str, optional
            ...
            the `name` parameter of the ``StiffPlate1D`` object will
            be used.
    ...
    def save(self):
        """Save the ``StiffPlate1D`` object using ``cPickle``

        Notes
        -----
        The pickled file will have the name stored in
        ``StiffPlate1D.name`` followed by a
        ``'.StiffPlate1D'`` extension.

        """
        name = self.name + '.StiffPlate1D'
        msg('Saving StiffPlate1D to {0}'.format(name))
